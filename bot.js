      ...
      case '~set':
        ...
        if (isNaN(minAge)) {
          ...
          client.channels.fetch(message.channel.id)
            .then(channel => channel.send(`The current minimum age is ${minAge} minutes. You can modify it using \`~set {Number of minutes}\``))
            .catch(console.error)
        } else {
          client.channels.fetch(message.channel.id)
            .then(channel => channel.send(`The minimum age has been set to ${minAge} minutes`))
            .catch(console.error)
        }
        break
      case '~exclude':
